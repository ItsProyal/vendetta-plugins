...
patches.push(after("generate", RowManager.prototype, ([data], row) => {
  ...
  for (let i = 0; i < content.length; i++) {
    ...
    if (!match) continue;
    const imageurl = `${match[0]}?size=128`;

    const url = new URL(el.target);

    let emojiname = EmojiStore.getCustomEmojiById(match[1])?.name ?? url?.searchParams.get("name") ?? "FakeNitroEmoji"

    content[i] = {
      type: "customEmoji",
      id: match[1],
      alt: `${emojiname}`,
      src: imageurl.replace("gif", "webp"),
      frozenSrc: imageurl,
      jumboable: jumbo,
    };
  }
